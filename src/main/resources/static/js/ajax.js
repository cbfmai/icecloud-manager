...
var common_ajax = function () {
    ...
    var ajaxMainPanel = function (url, data, callback) {
        var result = ajaxFunc(url, data, callback);
        result += '<script>$(function(){$("input").on("blur",function(e){$(this).val($.trim(this.value))})})</script>';
        var toolbarDiv = $("#maincontent");
        toolbarDiv.children().remove();
        toolbarDiv.html(result);
    };
